...
_URL shortening is a technique on the World Wide Web in which a Uniform Resource Locator (URL) may be made substantially shorter in length and still direct to the required page. This is achieved by using an HTTP Redirect on a domain name that is short, which links to the web page that has a long URL. For example, the URL http://en.wikipedia.org/wiki/URL_shortening can be shortened to http://bit.ly/urlwiki or http://tinyurl.com/urlwiki. _ (wikipedia)

Moreover, besides a giant lookup table, usually url shortner services offers user registration, tracking of statistics associated to each shorturl, such as the number of clicks per day, week, month and in total, and the country of origin of the requests.

Our objective is to design a simple schema for such a service using HBase.
...
Before looking at HBase, let us look at a traditional database design, with normalized relationships.
...
User action: Click on a shorturl
...
Users’ personal pages and shorturl counters
...
The previous query has a problem: the user cainarachi created 3 shortIds (http://goo.gl/313158, http://goo.gl/1483308 and http://goo.gl/681023), but the result shows just two shortIds. In fact, it does not output shortId with zero clicks, since the previous syntax (FROM t1, t2 WHERE t1.PK=t2.FK) implies an inner join: in fact, shortId “681023” has no clicks, when we look for click.shortId corresponding to shorturl.id, we do not find anything, so we do not output anything. We should have output a NULL instead. In this case we should use a “LEFT JOIN”. Let us see what happens when using a LEFT JOIN::
...
This query is rather complicated:
It requires 3 joins:
...
User Action: Shorturl statistics
...
R1. create 'user-test', {NAME => 'data', VERSIONS => 1}
...
R7. No, hbase.hregion.max.filesize is 256MB, while the data il ~37MB
...
R1. create 'url-test', {NAME => 'data', VERSION => 1}, {NAME => 'content', VERSION => 1}
